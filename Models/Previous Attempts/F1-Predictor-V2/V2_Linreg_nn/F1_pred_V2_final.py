...
def objective_mlp(trial):
    params = {
        ...
        'learning_rate_init': trial.suggest_float('learning_rate_init', 0.00001, 0.09951, step=0.0005),
    ...
    mlp_model.fit(xv2_train, y_train)
    scores = -cross_val_score(mlp_model, xv2_train, y_train, cv=tscv, scoring='neg_root_mean_squared_error')
    return scores.mean()
...
MLP_Predictor(test21, xv2_test21, 'MLP V2, 2021')
MLP_Predictor(test22, xv2_test22, 'MLP V2, 2022')

print(model_performance)
model_performance.to_excel('Model_Performance_V2_Final.xlsx', index=False)